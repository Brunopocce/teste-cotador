...
export const PriceSummaryTable: React.FC<PriceSummaryTableProps> = ({ groups }) => {
  ...
  // FIX for React Error #310: Extracted modal JSX into its own render function
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm" onClick={() => setSelectedGroup(null)}>
            <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full flex flex-col max-h-[90vh] animate-slideUp" onClick={e => e.stopPropagation()}>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6">
                    ...
                    <div>
                        ...
                        {basePlan.operator === 'Amhemed' ? (
                            ...
                            <ul className="text-sm text-gray-600 space-y-2 bg-gray-50 p-3 rounded-lg border border-gray-100">{basePlan.gracePeriods.map((gp, i) => (<li key={i} className="flex items-start gap-2"><div className="w-1.5 h-1.5 rounded-full bg-blue-400 mt-1.5 flex-shrink-0"></div><span>{gp}</span></li>))}</ul>
                        )}
                    </div>
                ...
                <div className="p-6 border-t border-gray-100 flex-shrink-0">
                    <button onClick={() => setSelectedGroup(null)} className="w-full bg-blue-900 text-white font-bold py-3 rounded-xl hover:bg-blue-800 transition-colors shadow-lg shadow-blue-900/20">Fechar</button>
                </div>
            </div>
        </div>
    );
  ...
  return (
    <div className="mt-16 mb-12 animate-fadeIn">
            ...
            {groups.map((group, idx) => {
              ...
              return (
                <tr 
                  key={idx} 
                  onClick={() => {
                    const initialTab = basePlan.operator === 'Fênix Medical' ? 'standard' : 'normal';
                    setGracePeriodTab(initialTab);
      ...
      {selectedGroup && createPortal(renderDetailsModal(), document.body)}
    </div>
  );
};